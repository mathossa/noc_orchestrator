...
test('navigates customer, site, device group and device detail with attention drill-down', async ({
  ...
  await expect(
    page
      .getByRole('link', { name: 'Playwright HQ', exact: true })
      .locator('xpath=ancestor::tr'),
  ).toContainText('1 critical · 1 attention')
  await page.getByRole('link', { name: 'Playwright HQ', exact: true }).click()
  await expect(page).toHaveURL(
    new RegExp(
      '/devices/customers/' + ids.customer + '/sites/' + ids.site + '$',
    ),
  )
  ...
  await expect(
    page.getByRole('heading', { name: 'Firmware', exact: true }),
  ).toBeVisible({ timeout: 20_000 })

  await expect(
    page.getByRole('link', { name: 'Playwright Switches', exact: true }),
  ).toHaveAttribute(
    'href',
    `/devices/customers/${ids.customer}/sites/${ids.site}/types/${ids.type}`,
  )
  ...
  await expect(edit.getByLabel('Device name', { exact: true })).toHaveValue(
    'PW-SW-01',
  )
  await edit
    .getByLabel('Management address', { exact: true })
    .fill('10.107.10.11')
  ...
  const updatedCustomerRow = page
    ...
    .locator('xpath=ancestor::tr')
  await updatedCustomerRow
    .getByRole('link', { name: /2 devices need attention/i })
    .click()
  await expect(page).toHaveURL(
    new RegExp('/devices/customers/' + ids.customer + '\\?attention=1$'),
  )
})
...
test('debounces scoped search and exposes a direct site link', async ({
  page,
}) => {
  ...
  page.on('request', (request) => {
    ...
    if (url.pathname === '/devices' && url.searchParams.has('q'))
  ...
  await expect(
    page.getByRole('link', {
      name: 'Site: Playwright Inventory Customer / Playwright HQ',
    }),
  ).toHaveAttribute(
    'href',
    `/devices/customers/${ids.customer}/sites/${ids.site}`,
...
test('uses the device workspace and records a note and issue without changing firmware', async ({
  page,
}) => {
