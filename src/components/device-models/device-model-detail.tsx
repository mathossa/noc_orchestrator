  ...
  return (
    <>
      <PageHeader
        ...
        title={model.model}
        description="Concrete model firmware lifecycle context. Family policy and compatibility may be inherited, while concrete-model rules can explicitly refine them."
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div className="min-w-0 space-y-5">
          <section id="desired-firmware-policy" className="scroll-mt-6 rounded-lg border border-[var(--border)] bg-[var(--surface)]">
            <SectionHeading title="Desired firmware policy" description="Desired state is explicit policy intent. A selected logical target is saved only when the compatibility engine can resolve a safe concrete firmware path for this model." />
            <div className="grid gap-px bg-[var(--border)] lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)]">
              ...
              <div className="bg-[var(--surface)] p-4">
                ...
                <p className="mt-2 text-xs leading-5 text-[var(--muted)]">Catalog choices are active policy-eligible releases from the same vendor. Saving requires compatibility to resolve exactly one safe concrete image/variant; unknown, ambiguous, or incompatible targets are rejected with an explanation.</p>
          ...
          <section className="rounded-lg border border-[var(--border)] bg-[var(--surface)]">
            <SectionHeading title="Firmware catalog candidates" description="Same-vendor catalog releases are visible here. Actual model/image compatibility is resolved by the compatibility engine and explained in the compatibility section below." />
            {model.availableFirmware.releases.length === 0 ? <div className="px-4 py-6 text-sm text-[var(--muted)]">No releases exist for {model.vendor.name}.</div> : <div className="divide-y divide-[var(--border)]">{model.availableFirmware.releases.map((release) => <Link key={release.id} href={`/firmware/${release.id}`} className="flex items-center justify-between gap-3 px-4 py-3 hover:bg-[var(--surface-raised)]"><span><span className="font-mono font-semibold text-[var(--accent-light)]">{release.version}</span><span className="ml-2 text-xs text-[var(--muted)]">{release.firmwareTrain?.name ?? release.platform}</span></span><span className="text-right text-xs text-[var(--muted)]">{release.status}{release.isActive ? '' : ' · archived'}{release.selectable ? ' · policy eligible' : ''}</span></Link>)}</div>}
          </section>
        ...
        <section className="h-fit rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4 sm:p-5">
          ...
          <dl className="mt-4 space-y-3 text-sm"><DetailRow label="Vendor" value={model.vendor.name} /><DetailRow label="Family / series" value={model.family?.name ?? '—'} /><DetailRow label="Device type" value={model.deviceType.name} /><DetailRow label="Observed platform hint" value={model.platform ?? '—'} /><DetailRow label="Status" value={model.isActive ? 'Active' : 'Archived'} /><DetailRow label="Catalog releases" value={model.availableFirmware.releases.length} /><DetailRow label="Source" value={model.source} /><DetailRow label="External provider" value={model.externalProvider ?? '—'} /><DetailRow label="External ID" value={model.externalId ?? '—'} /><DetailRow label="Last synchronized" value={model.lastSynchronizedAt ? new Date(model.lastSynchronizedAt).toLocaleString() : 'Never / manual'} /></dl>
          {model.family ? <div className="mt-5 rounded-md border border-[var(--border)] bg-[var(--surface-raised)] p-3 text-xs leading-5 text-[var(--muted-strong)]"><strong>{model.family.name}</strong> can provide inherited policy and compatibility. Concrete-model rules may refine or restrict that family baseline; the current platform field does not by itself decide desired compatibility.</div> : null}
