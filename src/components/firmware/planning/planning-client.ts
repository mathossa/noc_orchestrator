...
export function groupPreviewExceptions(
  targets: PreviewTarget[],
): PreviewExceptionGroup[] {
  const groups = new Map<string, PreviewExceptionGroup>()

  for (const target of targets) {
    if (
      !target.effectiveExceptionId ||
      (target.disposition !== 'ACTIVE_EXCEPTION' && !target.exceptionOverride)
    )
      continue

    const reason =
      target.effectiveExceptionReason ?? target.effectiveExceptionId
    const key = reason.trim().toLocaleLowerCase()
    const current = groups.get(key)
    if (current) {
      current.deviceIds.push(target.deviceId)
      current.count += 1
    } else {
      groups.set(key, {
        key,
  ...
  return [...groups.values()].sort((a, b) => a.reason.localeCompare(b.reason))
}
