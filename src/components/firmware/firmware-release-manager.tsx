...
type CatalogPayload = { data?: FirmwareReleaseRecord[]; meta?: { vendors?: FirmwareReleaseReference[] } } & ApiError
...
export function FirmwareReleaseManager() {
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      return [record.vendor.name, record.platform, record.version, record.filename ?? '']
        .join(' ')
        .toLocaleLowerCase('en-US')
        .includes(needle)
    })
  }, [records, search, vendorFilter, statusFilter, archiveFilter])
  ...
  return (
    <>
      <PageHeader
        ...
        description="Record releases that may be used by firmware policy. Catalog status and recency never make a release desired automatically."
      />

      <div className="mb-4 rounded-md border border-[var(--accent-muted)] bg-[var(--accent-soft)] px-4 py-3 text-sm text-[var(--accent-light)]">
        Latest is not desired. Engineers explicitly select desired firmware through policy in a separate workflow.
      </div>
      ...
      <form onSubmit={save} className="mb-5 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4 sm:p-5">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-[var(--muted)]">Version is stored as an opaque vendor string; no semantic-version ordering is assumed.</p>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <FormField label="Vendor" htmlFor="firmware-vendor" error={fieldErrors.vendorId}>
            <SelectInput id="firmware-vendor" value={form.vendorId} onChange={(e) => setForm({ ...form, vendorId: e.target.value })} required>
          ...
          <FormField label="Platform / family" htmlFor="firmware-platform" error={fieldErrors.platform}>
            <TextInput id="firmware-platform" value={form.platform} onChange={(e) => setForm({ ...form, platform: e.target.value })} placeholder="IOS XE" required />
          </FormField>
          <FormField label="Version" htmlFor="firmware-version" error={fieldErrors.version}>
            <TextInput id="firmware-version" value={form.version} onChange={(e) => setForm({ ...form, version: e.target.value })} placeholder="17.15.5" required />
          </FormField>
      ...
      <section className="rounded-lg border border-[var(--border)] bg-[var(--surface)]">
        <div className="grid gap-3 border-b border-[var(--border)] p-4 md:grid-cols-2 xl:grid-cols-4">
          <TextInput aria-label="Search firmware releases" type="search" placeholder="Search vendor, platform, version, filename…" value={search} onChange={(e) => setSearch(e.target.value)} />
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1050px] text-left text-sm">
              <thead className="border-b border-[var(--border)] bg-[var(--surface-raised)] text-xs uppercase tracking-[0.08em] text-[var(--muted)]">
                <tr><th className="px-4 py-3">Release</th><th className="px-4 py-3">Vendor</th><th className="px-4 py-3">Platform</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">File</th><th className="px-4 py-3">State</th><th className="px-4 py-3 text-right">Actions</th></tr>
              </thead>
