...
function initialDraft(
  item: DeviceImportUnresolvedReference,
  references: DeviceImportResolutionReferences,
): CreateDraft {
...
export function DeviceImportReferenceResolver({
  ...
  const activeDeviceTypes = useMemo(
    () => references.deviceTypes.filter((record) => record.isActive),
    [references.deviceTypes],
  )

  function choiceOptions(item: DeviceImportUnresolvedReference) {
    if (item.kind === 'DEVICE_TYPE') {
      return activeDeviceTypes.map((record) => ({ id: record.id, label: `${record.name} (${record.code})` }))
    }
  ...
  function openCreate(item: DeviceImportUnresolvedReference) {
    ...
    setDrafts((current) =>
      current[item.key] ? current : { ...current, [item.key]: initialDraft(item, references) },
    )
    setError(null)
    setMessage(null)
  }
  ...
  function updateDraft(key: string, patch: Partial<CreateDraft>) {
    setDrafts((current) => {
      ...
      return {
        ...current,
        [key]: { ...(current[key] ?? initialDraft(item, references)), ...patch },
      }
    })
  }
  ...
  async function remember(item: DeviceImportUnresolvedReference, targetId: string) {
    ...
    if (!response.ok) {
      throw new Error(payload.error?.message ?? 'The import match could not be remembered.')
    }
  }

  async function applyExistingResolution(item: DeviceImportUnresolvedReference, always: boolean) {
    ...
    try {
      ...
      setMessage(
        always
          ? `“${item.sourceValue}” will now match this record on future imports.`
    ...
    try {
      if (item.kind === 'DEVICE_TYPE') {
        ...
        if (!response.ok || !payload.data) {
          throw new Error(payload.error?.message ?? 'The device type could not be created.')
        }
        ...
        if (!response.ok || !payload.data) {
          throw new Error(payload.error?.message ?? 'The device model could not be created.')
        }
      ...
      setMessage(
        always
          ? `Created and remembered “${item.sourceValue}” for future imports.`
    ...
    <section className="border-b border-[var(--border)] bg-[var(--surface-raised)]">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--border)] px-4 py-4 sm:px-5">
        ...
        <div className="text-xs text-[var(--muted-strong)]">
          {resolvedCount} of {unresolvedCount} values resolved for this preview
        </div>
      </div>

      {error ? (
        <div className="mx-4 mt-4 rounded-md border border-[#754040] bg-[#2a1b1b] px-3 py-2 text-sm text-[#f0b0b0] sm:mx-5">
          {error}
        </div>
      ) : null}
      {message ? (
        <div className="mx-4 mt-4 rounded-md border border-[#285f48] bg-[#142b22] px-3 py-2 text-sm text-[#a9e8c6] sm:mx-5">
          {message}
        </div>
      ) : null}
      ...
      <div className="divide-y divide-[var(--border)]">
        {unresolved.map((item) => {
          ...
          const draft = drafts[item.key] ?? initialDraft(item, references)
          const families = references.families.filter(
            (family) => family.isActive && family.vendorId === draft.vendorId,
          )
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="text-xs font-semibold uppercase tracking-[0.08em] text-[var(--accent-light)]">
                    {item.kind === 'DEVICE_TYPE' ? 'Device type' : 'Concrete device model'}
                  </div>
                  <div className="mt-1 font-mono text-sm font-semibold text-[var(--foreground)]">
                    {item.sourceValue}
                  </div>
                  <div className="mt-1 text-xs text-[var(--muted)]">
                    Rows {item.rowNumbers.join(', ')}
                    {item.vendorName ? ` · Vendor: ${item.vendorName}` : ''}
                  </div>
                </div>
                {resolutions[item.key] ? (
                  <span className="rounded border border-[#285f48] bg-[#142b22] px-2 py-1 text-xs font-semibold text-[#a9e8c6]">
                    Resolution selected
                  </span>
                ) : null}
              </div>
              ...
              <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto_auto_auto] lg:items-end">
                <FormField label="Link to existing" htmlFor={`resolve-${item.key}`}>
                  <SelectInput
                    ...
                    value={selected}
                    onChange={(event) =>
                      setChoices((current) => ({ ...current, [item.key]: event.target.value }))
                    }
                  >
                    <option value="">
                      Choose configured {item.kind === 'DEVICE_TYPE' ? 'device type' : 'model'}
                    </option>
                    {options.map((option) => (
                ...
                <Button
                  type="button"
                  variant="ghost"
                  disabled={!selected || busyKey === item.key || disabled}
                  onClick={() => void applyExistingResolution(item, false)}
                >
                  Use once
                </Button>
                <Button
                  ...
                  variant="ghost"
                  disabled={!selected || busyKey === item.key || disabled}
                  onClick={() => void applyExistingResolution(item, true)}
                >
                  Always match
                </Button>
                <Button
                  type="button"
                  variant="primary"
                  disabled={busyKey === item.key || disabled}
                  onClick={() => openCreate(item)}
                >
                  {createKey === item.key ? 'Close create' : 'Create new'}
                </Button>
              </div>
                ...
                <div className="mt-4 rounded-md border border-[var(--border)] bg-[var(--surface)] p-4">
                  {item.kind === 'DEVICE_TYPE' ? (
                    <div className="grid gap-3 md:grid-cols-2">
                      <FormField label="Type name" htmlFor={`create-type-name-${item.key}`}>
                        <TextInput
                          id={`create-type-name-${item.key}`}
                          value={draft.name}
                          onChange={(event) => updateDraft(item.key, { name: event.target.value })}
                        />
                      </FormField>
                      <FormField label="Type code" htmlFor={`create-type-code-${item.key}`}>
                        <TextInput
                          id={`create-type-code-${item.key}`}
                          value={draft.code}
                          onChange={(event) => updateDraft(item.key, { code: event.target.value })}
                        />
                      </FormField>
                    </div>
                  ) : (
                    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                      <FormField label="Vendor" htmlFor={`create-model-vendor-${item.key}`}>
                        <SelectInput
                          id={`create-model-vendor-${item.key}`}
                          value={draft.vendorId}
                          onChange={(event) =>
                            updateDraft(item.key, { vendorId: event.target.value, familyId: '' })
                          }
                        >
                      ...
                      <FormField label="Device type" htmlFor={`create-model-type-${item.key}`}>
                        <SelectInput
                          id={`create-model-type-${item.key}`}
                          value={draft.deviceTypeId}
                          onChange={(event) => updateDraft(item.key, { deviceTypeId: event.target.value })}
                        >
                      ...
                      <FormField
                        label="Family / series"
                        htmlFor={`create-model-family-${item.key}`}
                        description="Optional; no firmware inheritance is implied."
                      >
                        <SelectInput
                          id={`create-model-family-${item.key}`}
                          value={draft.familyId}
                          onChange={(event) => updateDraft(item.key, { familyId: event.target.value })}
                      ...
                      <FormField
                        label="Platform"
                        htmlFor={`create-model-platform-${item.key}`}
                        description="Optional firmware compatibility platform."
                      >
                        <TextInput
                          id={`create-model-platform-${item.key}`}
                          value={draft.platform}
                          onChange={(event) => updateDraft(item.key, { platform: event.target.value })}
                        />
                      </FormField>
                      <div className="md:col-span-2 xl:col-span-4">
                        <FormField
                          label="Concrete model name"
                          htmlFor={`create-model-name-${item.key}`}
                          description="Defaults to the exact spreadsheet notation; vendor prefixes are not stripped automatically."
                        >
                          <TextInput
                            id={`create-model-name-${item.key}`}
                            value={draft.model}
                            onChange={(event) => updateDraft(item.key, { model: event.target.value })}
                          />
                        </FormField>
                      </div>
                    </div>
                  ...
                  <div className="mt-4 flex flex-wrap justify-end gap-2">
                    <Button
                      ...
                      variant="ghost"
                      disabled={busyKey === item.key || disabled}
                      onClick={() => void createReference(item, false)}
                    >
                      Create + use once
                    </Button>
                    <Button
                      ...
                      variant="primary"
                      disabled={busyKey === item.key || disabled}
                      onClick={() => void createReference(item, true)}
                    >
                      Create + always match
                    </Button>
                  </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] px-4 py-4 sm:px-5">
        <p className="text-xs text-[var(--muted)]">
          Re-run validation after making resolutions. Saved aliases are applied automatically on future XLSX previews.
        </p>
        <Button type="button" variant="primary" onClick={onRepreview} disabled={disabled}>
          Re-run preview with resolutions
        </Button>
      </div>
    </section>
  )
