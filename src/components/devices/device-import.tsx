  ...
  return (
    <>
      <PageHeader
        ...
        description="Inspect, map and validate an existing spreadsheet before device inventory is written. Unknown types/models can be linked, remembered, or deliberately created during resolution."
      ...
      {inspection && currentSheet ? (
        <div className="mt-5 space-y-5">
          <section className="rounded-lg border border-[var(--border)] bg-[var(--surface)]">
            <div className="border-b border-[var(--border)] px-4 py-3 sm:px-5"><h2 className="text-sm font-semibold">2. Worksheet and header</h2><p className="mt-1 text-xs leading-5 text-[var(--muted)]">Choose the useful worksheet and confirm which row contains the column headings.</p></div>
            <div className="grid gap-4 p-4 sm:grid-cols-2 sm:p-5">
              ...
              <FormField label="Header row" htmlFor="device-import-header" description={`Automatically detected row ${currentSheet.detectedHeaderRow}.`}><SelectInput id="device-import-header" value={String(headerRow)} onChange={(event) => changeHeaderRow(Number(event.target.value))}>{currentSheet.previewRows.map((row) => <option key={row.rowNumber} value={row.rowNumber}>Row {row.rowNumber}: {row.values.filter(Boolean).slice(0, 4).join(' · ') || '(blank)'}</option>)}</SelectInput></FormField>
            </div>
            <div className="border-t border-[var(--border)] px-4 py-3 sm:px-5">
              ...
              <div className="noc-scrollbar overflow-x-auto rounded-md border border-[var(--border)]"><table className="min-w-full text-left text-xs"><tbody className="divide-y divide-[var(--border)]">{currentSheet.previewRows.slice(0, 10).map((row) => <tr key={row.rowNumber} className={row.rowNumber === headerRow ? 'bg-[var(--accent-soft)]' : ''}><th className="sticky left-0 bg-[var(--surface-raised)] px-2 py-1.5 font-semibold text-[var(--muted)]">{row.rowNumber}</th>{Array.from({ length: currentSheet.columnCount }, (_unused, index) => <td key={index} className="max-w-[220px] truncate whitespace-nowrap px-2 py-1.5 text-[var(--muted-strong)]">{row.values[index] ?? ''}</td>)}</tr>)}</tbody></table></div>
            </div>
          </section>

          <section className="rounded-lg border border-[var(--border)] bg-[var(--surface)]">
            <div className="border-b border-[var(--border)] px-4 py-3 sm:px-5"><h2 className="text-sm font-semibold">3. Column mapping</h2><p className="mt-1 text-xs leading-5 text-[var(--muted)]">Automatic suggestions use common English/Dutch inventory headings. Every destination field can be mapped only once.</p></div>
          ...
          <section className="rounded-lg border border-[var(--border)] bg-[var(--surface)]">
            <div className="border-b border-[var(--border)] px-4 py-3 sm:px-5"><h2 className="text-sm font-semibold">4. File-level defaults</h2><p className="mt-1 text-xs leading-5 text-[var(--muted)]">Use these when the workbook belongs to one customer/site or when external IDs share one source system.</p></div>
            <div className="grid gap-4 p-4 md:grid-cols-3 sm:p-5">
              <FormField label="Default customer" htmlFor="device-import-customer" description={mappedFieldSet.has('customer') ? 'Used only when the mapped customer cell is blank.' : 'Required for new devices when Customer is not mapped.'}><SelectInput id="device-import-customer" value={defaultCustomerId} onChange={(event) => { setDefaultCustomerId(event.target.value); if (!references.sites.some((site) => site.id === defaultSiteId && site.customerId === event.target.value)) setDefaultSiteId(''); invalidatePreview() }}><option value="">No default customer</option>{references.customers.map((customer) => <option key={customer.id} value={customer.id} disabled={!customer.isActive}>{customer.name}{customer.code ? ` (${customer.code})` : ''}{customer.isActive ? '' : ' · archived'}</option>)}</SelectInput></FormField>
              <FormField label="Default site" htmlFor="device-import-site" description="Optional. A site can never be assigned across customers."><SelectInput id="device-import-site" value={defaultSiteId} onChange={(event) => { setDefaultSiteId(event.target.value); invalidatePreview() }} disabled={!defaultCustomerId}><option value="">No default site</option>{defaultSites.map((site) => <option key={site.id} value={site.id} disabled={!site.isActive}>{site.name}{site.code ? ` (${site.code})` : ''}{site.isActive ? '' : ' · archived'}</option>)}</SelectInput></FormField>
              <FormField label="Default external provider" htmlFor="device-import-provider" description="Optional but recommended when mapping External ID for deterministic updates."><TextInput id="device-import-provider" value={externalProvider} onChange={(event) => { setExternalProvider(event.target.value); invalidatePreview() }} placeholder="e.g. CMDB export" /></FormField>
            </div>
            <div className="flex justify-end border-t border-[var(--border)] px-4 py-3 sm:px-5"><Button variant="primary" onClick={() => void previewImport()} disabled={busy !== null}>{busy === 'preview' ? 'Validating…' : 'Validate and preview'}</Button></div>
          </section>
        </div>
      ) : null}
      ...
      {preview ? (
        <section className="mt-5 rounded-lg border border-[var(--border)] bg-[var(--surface)]">
          <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--border)] px-4 py-3 sm:px-5">
            <div><h2 className="text-sm font-semibold">5. Import preview</h2><p className="mt-1 text-xs leading-5 text-[var(--muted)]">Nothing has been written to device inventory yet. Resolve unknown reference values below, then re-run validation.</p></div>
            <div className="flex flex-wrap gap-2 text-xs"><PreviewCount label="Create" value={preview.counts.create} /><PreviewCount label="Update" value={preview.counts.update} /><PreviewCount label="Unchanged" value={preview.counts.unchanged} /><PreviewCount label="Conflict" value={preview.counts.conflict} /><PreviewCount label="Error" value={preview.counts.error} /></div>
          </div>

          <DeviceImportReferenceResolver
            unresolved={preview.unresolvedReferences}
            references={references}
            resolutions={resolutions}
            onResolutionChange={(key, targetId) => setResolutions((current) => ({ ...current, [key]: targetId }))}
            onDeviceTypeCreated={(record) => setReferences((current) => ({ ...current, deviceTypes: [...current.deviceTypes, record] }))}
            onModelCreated={(record) => setReferences((current) => ({ ...current, models: [...current.models, record] }))}
            onRepreview={() => void previewImport()}
            disabled={busy !== null}
          />

          <div className="flex flex-wrap items-end justify-between gap-3 border-b border-[var(--border)] px-4 py-3 sm:px-5">
            <FormField label="Show" htmlFor="device-import-filter"><SelectInput id="device-import-filter" value={filter} onChange={(event) => setFilter(event.target.value as 'ALL' | DeviceImportAction)}><option value="ALL">All preview rows</option>{Object.entries(ACTION_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</SelectInput></FormField>
            <div className="text-sm text-[var(--muted-strong)]">{selectedRows.length} of {importableRows.length} importable rows selected</div>
          </div>

          <div className="noc-scrollbar overflow-x-auto"><table className="w-full min-w-[1250px] text-left text-sm"><thead className="border-b border-[var(--border)] bg-[var(--surface-raised)] text-xs uppercase tracking-[0.08em] text-[var(--muted)]"><tr><th className="w-12 px-3 py-2.5"><input type="checkbox" aria-label="Select visible importable rows" checked={allVisibleSelected} onChange={(event) => toggleVisibleRows(event.target.checked)} disabled={allVisibleImportable.length === 0} /></th><th className="px-3 py-2.5">Row</th><th className="px-3 py-2.5">Action</th><th className="px-3 py-2.5">Identity</th><th className="px-3 py-2.5">Customer / site</th><th className="px-3 py-2.5">Model</th><th className="px-3 py-2.5">Current firmware</th><th className="px-3 py-2.5">Changes / validation</th></tr></thead><tbody className="divide-y divide-[var(--border)]">{visibleRows.map((row) => <tr key={row.rowNumber} className={row.action === 'ERROR' || row.action === 'CONFLICT' ? 'bg-[#2a1b1b]/35' : ''}><td className="px-3 py-3"><input type="checkbox" checked={selectedRows.includes(row.rowNumber)} onChange={() => toggleRow(row.rowNumber)} disabled={!row.importable} aria-label={`Import spreadsheet row ${row.rowNumber}`} /></td><td className="px-3 py-3 tabular-nums text-[var(--muted)]">{row.rowNumber}</td><td className="px-3 py-3"><span className="rounded border border-[var(--border-strong)] px-2 py-1 text-xs font-semibold">{ACTION_LABELS[row.action]}</span></td><td className="px-3 py-3"><div className="font-semibold text-[var(--foreground)]">{row.identity}</div>{row.existingDeviceId ? <Link href={`/devices/${row.existingDeviceId}`} className="mt-1 inline-flex text-xs text-[var(--accent-light)] hover:underline">Existing device</Link> : null}</td><td className="px-3 py-3 text-[var(--muted-strong)]"><div>{row.customer ?? '—'}</div><div className="mt-1 text-xs text-[var(--muted)]">{row.site ?? 'No site'}</div></td><td className="px-3 py-3 text-[var(--muted-strong)]">{row.model ?? '—'}</td><td className="px-3 py-3 font-mono text-[var(--muted-strong)]">{row.currentFirmware ?? 'Unknown'}</td><td className="px-3 py-3">{row.issues.length > 0 ? <div className="space-y-1">{row.issues.map((issue, index) => <div key={`${issue.message}-${index}`} className={issue.level === 'error' ? 'text-xs text-red-300' : 'text-xs text-amber-200'}>{issue.message}</div>)}</div> : null}{row.changes.length > 0 ? <details className="mt-1"><summary className="cursor-pointer text-xs font-semibold text-[var(--accent-light)]">{row.changes.length} change{row.changes.length === 1 ? '' : 's'}</summary><div className="mt-2 space-y-1 text-xs text-[var(--muted)]">{row.changes.map((change) => <div key={change.field}><strong className="text-[var(--muted-strong)]">{change.label}:</strong> {change.before ?? '—'} → {change.after ?? '—'}</div>)}</div></details> : row.issues.length === 0 ? <span className="text-xs text-[var(--muted)]">No inventory changes</span> : null}</td></tr>)}</tbody></table></div>

          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] px-4 py-4 sm:px-5"><p className="max-w-3xl text-xs leading-5 text-[var(--muted)]">Import updates inventory/current-firmware fields only. Desired firmware policy, lifecycle decisions, planning and existing audit history are not imported or replaced.</p><Button variant="primary" onClick={() => void commitImport()} disabled={selectedRows.length === 0 || busy !== null}>{busy === 'commit' ? 'Importing…' : `Import ${selectedRows.length} selected`}</Button></div>
        </section>
      ) : null}

      {result ? <section className="mt-5 rounded-lg border border-[#285f48] bg-[#142b22] p-5"><h2 className="text-sm font-semibold text-[#c8f3da]">Import completed</h2><div className="mt-4 grid gap-3 sm:grid-cols-4"><ResultStat label="Created" value={result.created} /><ResultStat label="Updated" value={result.updated} /><ResultStat label="Skipped" value={result.skipped} /><ResultStat label="Failed / excluded" value={result.failed} /></div><div className="mt-4 flex flex-wrap gap-2"><Link href="/devices?source=IMPORT" className="rounded-md border border-[#4a8b6c] bg-[#1b382c] px-3 py-2 text-sm font-semibold text-[#c8f3da] hover:bg-[#234535]">View imported devices</Link><Button variant="ghost" onClick={() => { setPreview(null); setResult(null); setSelectedRows([]) }}>Prepare another preview</Button></div></section> : null}
    </>
  )
}
