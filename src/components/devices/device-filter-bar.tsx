...
export function DeviceFilterBar({ meta }: { meta: DeviceQueryMeta }) {
  ...
  const urlValue = (name: ParamName, fallback = '') => searchParams.get(name) ?? fallback
  const ui = {
    q: urlValue('q'),
    ...
    deviceType: urlValue('deviceType'),
    contract: urlValue('contract'),
    ...
    groupBy: urlValue('groupBy', 'none'),
    sort: urlValue('sort', 'customer'),
    direction: urlValue('direction', 'asc'),
    pageSize: urlValue('pageSize', '50'),
  }
  ...
  addChip('deviceType', 'Type', meta.deviceTypes.find((item) => item.id === ui.deviceType)?.name ?? ui.deviceType)
  addChip('contract', 'Contract', ui.contract === 'none' ? 'No effective contract' : (meta.contractTypes.find((item) => item.id === ui.contract)?.name ?? ui.contract))
    ...
    <div className="border-b border-[var(--border)] bg-[var(--surface)]">
      ...
      <details className="border-t border-[var(--border)] px-4 py-3" open={Boolean(ui.deviceType || ui.contract || ui.currentFirmware || ui.desiredFirmware || ui.technicalState || ui.workflow || ui.source || ui.archive !== 'active')}>
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <SelectInput aria-label="Filter devices by desired firmware" value={ui.desiredFirmware} onChange={(event) => setParam('desiredFirmware', event.target.value)}><option value="">All desired firmware</option><option value="none">No desired policy</option>{meta.firmwareReleases.map((item) => <option key={item.id} value={item.id}>{vendorById.get(item.vendorId) ?? item.platform} · {item.version}</option>)}</SelectInput>
          <SelectInput aria-label="Filter devices by technical state" value={ui.technicalState} onChange={(event) => setParam('technicalState', event.target.value)}><option value="">All technical states</option><option value="CURRENT">Current</option><option value="ACTION_REQUIRED">Action required</option><option value="UNKNOWN">Unknown</option><option value="NO_POLICY">No policy</option></SelectInput>
          <SelectInput aria-label="Filter devices by workflow state" value={ui.workflow} onChange={(event) => setParam('workflow', event.target.value)}><option value="">All workflow states</option><option value="PLANNED">Planned</option><option value="IGNORED">Ignored</option><option value="CUSTOMER_DECLINED">Customer declined</option><option value="DONE">Done</option><option value="UNDECIDED">No decision</option></SelectInput>
      ...
      {chips.length > 0 ? (
        <div className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] px-4 py-3">
          {chips.map((chip) => (
            <button key={chip.key} type="button" onClick={() => setParam(chip.key, '')} className="rounded-full border border-[var(--border-strong)] bg-[var(--surface-raised)] px-2.5 py-1 text-xs text-[var(--muted-strong)] hover:border-[var(--accent-muted)]"><span className="font-semibold">{chip.label}:</span> {chip.value} <span aria-hidden="true">×</span></button>
          ))}
