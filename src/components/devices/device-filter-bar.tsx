    ...
    <div className="border-b border-[var(--border)] bg-[var(--surface)]">
      ...
      <details className="border-t border-[var(--border)] px-4 py-3" open={Boolean(ui.deviceType || ui.contract || ui.currentFirmware || ui.desiredFirmware || ui.technicalState || ui.exceptionState || ui.exceptionReason || ui.exceptionScope || ui.workflow || ui.source || ui.archive !== 'active')}>
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <SelectInput aria-label="Filter devices by planning state" value={ui.workflow} onChange={(event) => setParam('workflow', event.target.value)}><option value="">All planning states</option><option value="PROPOSED">Proposed</option><option value="AWAITING_CUSTOMER">Awaiting customer</option><option value="APPROVED">Approved</option><option value="SCHEDULED">Scheduled</option><option value="IN_PROGRESS">In progress</option><option value="DONE">Done history</option><option value="CANCELLED">Cancelled history</option><option value="NOT_PLANNED">No active plan</option></SelectInput>
