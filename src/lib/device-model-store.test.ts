...
describe('device model persistence rules', () => {
  ...
  it('creates a manual model with valid vendor and device type references', async () => {
    ...
    expect(mocks.deviceModelCreate).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ vendorId: 'vendor-1', deviceTypeId: 'type-1', model: 'C9300-24P', source: 'MANUAL' }) }))
  ...
  it('scopes model uniqueness to the selected vendor', async () => {
    ...
    mocks.deviceModelCreate.mockResolvedValue({ ...baseRecord, id: 'model-2', vendorId: 'vendor-2', vendor: { id: 'vendor-2', code: 'OTHER', name: 'Other Vendor', isActive: true } })
  ...
  it('supports partial archive updates without overwriting model identity', async () => {
    mocks.deviceModelFindUnique.mockResolvedValue({ id: 'model-1', vendorId: 'vendor-1', deviceTypeId: 'type-1', model: 'C9300-24P', platform: 'Catalyst 9300', notes: null, isActive: true, source: 'MANUAL', externalProvider: null, externalId: null })
  ...
  it('aggregates usage and exposes catalog releases matching vendor/platform', async () => {
    ...
    mocks.firmwareReleaseFindMany.mockResolvedValue([
      { id: 'fw-1', version: '17.15.5', platform: 'Catalyst 9300', status: 'APPROVED', isActive: true, releasedAt: now },
    ])

    const result = await getDeviceModel('model-1')

    expect(result.availableFirmware).toEqual({
      available: true,
      releases: [{ id: 'fw-1', version: '17.15.5', platform: 'Catalyst 9300', status: 'APPROVED', isActive: true, releasedAt: now.toISOString() }],
    })
