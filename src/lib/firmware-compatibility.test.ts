...
function modelPlatformRule(platform: string, overrides: Partial<FirmwareCompatibilityRule> = {}): FirmwareCompatibilityRule {
  return rule({
    id: `model-platform-${platform}`,
    deviceModelFamilyId: null,
    deviceModelId: 'model-1',
    platform,
    sourceType: 'CONFIGURED_RULE',
    explanation: 'Supported platform selected on the device model.',
    ...overrides,
  })
}
...
describe('generic firmware compatibility evaluator', () => {
  ...
  it('treats the concrete model supported-platform selection as an allow-list', () => {
    const rules = [modelPlatformRule('AOS-S')]
    const aosS = evaluateFirmwareCompatibility({ model, release: release({ platform: 'AOS-S' }), rules })
    const futurePlatform = evaluateFirmwareCompatibility({ model, release: release({ platform: 'AOS-S-v2' }), rules })

    expect(aosS.status).toBe('COMPATIBLE')
    expect(futurePlatform).toMatchObject({
      status: 'INCOMPATIBLE',
      provenance: { kind: 'MODEL_RULE', sourceType: 'CONFIGURED_RULE' },
    })
    expect(futurePlatform.provenance.explanation).toContain('not selected as supported')
  })

  it('does not let inherited family evidence broaden an explicit concrete supported-platform selection', () => {
    const result = evaluateFirmwareCompatibility({
      model,
      release: release({ platform: 'AOS-S-v2' }),
      rules: [
        modelPlatformRule('AOS-S'),
        rule({ id: 'family-v2', platform: 'AOS-S-v2', decision: 'ALLOW' }),
      ],
    })
    expect(result.status).toBe('INCOMPATIBLE')
    expect(result.provenance.kind).toBe('MODEL_RULE')
  })

  it('allows a more-specific concrete rule to deliberately refine the supported-platform selection', () => {
    const result = evaluateFirmwareCompatibility({
      model,
      release: release({ id: 'v2-special', platform: 'AOS-S-v2' }),
      rules: [
        modelPlatformRule('AOS-S'),
        modelPlatformRule('AOS-S-v2', {
          id: 'model-v2-special',
          firmwareReleaseId: 'v2-special',
          explanation: 'This exact release is validated for the model.',
        }),
      ],
    })
    expect(result).toMatchObject({ status: 'COMPATIBLE', provenance: { id: 'model-v2-special' } })
  })

  it('allows one model to support multiple software platforms through data only', () => {
    const rules = [modelPlatformRule('AOS-8'), modelPlatformRule('AOS-10')]
    const aos8 = evaluateFirmwareCompatibility({ model, release: release({ platform: 'AOS-8' }), rules })
    const aos10 = evaluateFirmwareCompatibility({ model, release: release({ platform: 'AOS-10' }), rules })
