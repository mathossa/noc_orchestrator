...
} satisfies Prisma.ImporterV2WorkspaceBatchSelect
...
function proposalContext(
  ...
  const key = (name: string) => targetId(snapshot, name) ?? targetLabel(snapshot, name)
  switch (field) {
    case 'businessUnit':
      return { customer: key('customer') }
    case 'site':
      return { customer: key('customer'), businessUnit: key('businessUnit') }
    case 'productFamily':
    ...
    case 'currentFirmware':
      return { vendor: key('vendor'), softwarePlatform: key('softwarePlatform') }
    default:
      return {}
  }
}
...
async function publishRow(input: {
  ...
  let created = false
  let beforeFirmware: { releaseId: string | null; rawVersion: string | null } | null = null
  if (canonicalDeviceId) {
    ...
    if (allowed.has('model')) data.deviceModel = { connect: { id: deviceModelId } }
    Object.assign(
      data,
      importerV2OwnedDeviceScalarPatch({
