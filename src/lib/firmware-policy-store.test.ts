...
describe('model desired firmware policy persistence', () => {
  beforeEach(() => {
    ...
    mocks.policyFindFirst.mockResolvedValue(policyRecord(approvedRelease, 'policy-new'))
  ...
  it('changes policy by preserving the old row as inactive and creating a new exact target', async () => {
    ...
    expect(mocks.policyCreate).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({ targetFirmwareReleaseId: 'fw-1' }),
    }))
    expect(mocks.auditCreate).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({
  ...
  it('accepts RECOMMENDED as a normal desired target', async () => {
    ...
    mocks.policyCreate.mockResolvedValue({ id: 'policy-rec', targetFirmwareReleaseId: 'fw-rec' })
    mocks.policyFindFirst.mockResolvedValue(policyRecord(recommended, 'policy-rec'))
  ...
  it('allows vendor-compatible releases when the model has no platform/family', async () => {
    ...
    await expect(setModelDesiredFirmwarePolicy('model-1', 'fw-1')).resolves.toMatchObject({
      release: expect.objectContaining({ version: '17.15.5' }),
    })
  })
  ...
  it('applies one exact release to multiple compatible models in one transaction', async () => {
    ...
    expect(mocks.auditCreate).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({ metadata: expect.objectContaining({ bulk: true }) }),
    }))
  })
  ...
  it('blocks setting firmware across a mixed-vendor selection', async () => {
    mocks.modelFindMany.mockResolvedValue([
      model,
      { id: 'model-2', vendorId: 'vendor-2', platform: 'Catalyst 9300', model: 'Other' },
    ])

    await expect(bulkSetModelDesiredFirmwarePolicies(['model-1', 'model-2'], 'fw-1'))
  ...
  it('blocks a target that is not compatible with every selected concrete platform', async () => {
    mocks.modelFindMany.mockResolvedValue([
      model,
      { id: 'model-2', vendorId: 'vendor-1', platform: 'Catalyst 9200', model: 'C9200-24P' },
    ])
  ...
  it('clears desired firmware for multiple selected models atomically', async () => {
    ...
    mocks.modelFindMany.mockResolvedValue([model, second])
    mocks.policyFindMany.mockResolvedValue([
      policyRecord(approvedRelease, 'policy-1', 'model-1'),
      policyRecord({ ...approvedRelease, id: 'fw-2', vendorId: 'vendor-2', platform: 'Other' }, 'policy-2', 'model-2'),
    ])
