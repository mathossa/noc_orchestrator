...
const unknownItem = {
  ...
  model: 'AP-515',
  status: 'UNKNOWN',
  explanation: 'No compatibility evidence.',
}
const incompatibleItem = {
  deviceModelId: 'model-1',
  model: 'AP-515',
  status: 'INCOMPATIBLE',
  explanation: 'Explicitly unsupported.',
}

const resolvedImpact = {
  canApply: true,
  reviewRequired: false,
  results: [resolvedItem],
...
const incompatibleImpact = {
  canApply: false,
  reviewRequired: false,
  results: [incompatibleItem],
  resolved: [],
  ambiguous: [],
...
describe('model desired firmware policy compatibility persistence', () => {
  ...
  it('allows desired policy intent when compatibility is unresolved but blocks explicit incompatibility', async () => {
    ...
    await expect(setModelDesiredFirmwarePolicy('model-1', 'fw-1')).resolves.toMatchObject({ release: expect.objectContaining({ id: 'fw-1' }) })
  ...
  it('applies one logical release to multiple models unless one is explicitly incompatible', async () => {
    const second = { id: 'model-2', vendorId: 'vendor-1', platform: 'AOS-8', model: 'AP-505' }
    mocks.modelFindMany.mockResolvedValue([model, second])
    const secondResolved = { deviceModelId: 'model-2', model: 'AP-505', status: 'RESOLVED', explanation: 'Resolved.' }
    mocks.previewLogicalTarget.mockResolvedValue({ ...resolvedImpact, results: [resolvedItem, secondResolved], resolved: [resolvedItem, secondResolved] })
    ...
    mocks.previewLogicalTarget.mockResolvedValue({
      ...incompatibleImpact,
      results: [resolvedItem, { ...incompatibleItem, deviceModelId: 'model-2', model: 'AP-505' }],
      resolved: [resolvedItem],
      incompatible: [{ ...incompatibleItem, deviceModelId: 'model-2', model: 'AP-505' }],
    })
...
describe('scoped policy foundation persistence', () => {
  ...
  it('persists a RANGE family track after compatibility preview', async () => {
  ...
  it('allows family policy intent with unknown children but rejects explicit incompatibility', async () => {
    mocks.releaseFindMany.mockResolvedValue([allowedRelease])
    mocks.policyCreate.mockResolvedValue(policyRecord(allowedRelease, 'policy-unknown', null, { deviceModelFamilyId: 'family-1' }))

    mocks.previewLogicalTarget.mockResolvedValue(unknownImpact)
    await expect(appendFirmwarePolicyVersion({ deviceModelFamilyId: 'family-1', policyMode: 'EXACT', desiredPlatform: 'AOS-10', targetFirmwareReleaseId: 'fw-1' })).resolves.toMatchObject({ id: 'policy-unknown' })
