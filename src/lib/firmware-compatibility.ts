...
function broadPlatformRule(rule: FirmwareCompatibilityRule) {
  return !rule.firmwareTrainId && !rule.logicalVersion && !rule.firmwareReleaseId && !rule.imageCode
}
...
export function evaluateFirmwareCompatibility(input: {
  ...
  const activeSubjectRules = rules
  ...
  const candidates = activeSubjectRules
  ...
  const concreteMatching = candidates.filter((candidate) => candidate.subjectSpecificity === 1)
  const configuredModelPlatformSelection = activeSubjectRules.filter(
    (rule) =>
      rule.deviceModelId === model.id
      && rule.sourceType === 'CONFIGURED_RULE'
      && rule.decision === 'ALLOW'
      && broadPlatformRule(rule),
  )

  // Once a concrete model has an explicit broad supported-platform selection,
  // that selection acts as an allow-list. This matters not only for platforms
  // known when the model was saved, but also for a new same-vendor platform that
  // may be added to the catalog later. A more-specific concrete rule can still
  // deliberately allow such a target; inherited family evidence cannot broaden
  // a concrete model's explicit platform selection.
  if (concreteMatching.length === 0 && configuredModelPlatformSelection.length > 0) {
    const supported = configuredModelPlatformSelection
    ...
    return {
      ...
      provenance: {
        kind: 'MODEL_RULE',
        id: configuredModelPlatformSelection[0]?.id ?? null,
        sourceType: 'CONFIGURED_RULE',
        explanation: `Platform ${release.platform} is not selected as supported for this model. Supported platform${supported.length === 1 ? '' : 's'}: ${supported.join(', ')}.`,
        inherited: false,
      },
      matchedRuleIds: configuredModelPlatformSelection.map((rule) => rule.id).sort(),
    }
  }
