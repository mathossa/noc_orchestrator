...
import {
  suggestImportModelFamily,
  suggestNewImportModelFamilyName,
} from '@/lib/device-import-model-family'
import {
  classifyImportedDeviceModel,
  softwarePlatformLegacyValue,
  type StoredDeviceImportNormalizationRule,
...
function metadata(value: unknown): DeviceImportStagedReferenceMetadata {
  return typeof value === 'object' && value !== null
    ? (value as DeviceImportStagedReferenceMetadata)
    : {}
}
...
async function assertMutableBatch(batchId: string) {
  ...
  if (batch.status === 'PUBLISHED')
    throw new DeviceImportStagingError(
      'Published import batches can no longer be changed.',
    )
  return batch
}
...
export async function getDeviceImportModelAssist(batchId: string) {
  ...
  const [models, families, vendors, deviceTypes, profileRules] =
    await Promise.all([
      targetIds.length
      ...
      batch.profileId
        ? (prisma.deviceImportProfileRule.findMany({
            ...
            Array<
              DeviceImportPredictionRule & StoredDeviceImportNormalizationRule
            >
          >)
        : Promise.resolve([]),
    ])
  const normalizationRules = profileRules.filter(
    (rule) => rule.action === 'NORMALIZE' && rule.field === 'model',
  )
  const predictionRules = profileRules.filter(
    (rule) => rule.action === 'PREDICT',
  )
  const vendorById = new Map(vendors.map((vendor) => [vendor.id, vendor]))
  const typeById = new Map(
    deviceTypes.map((deviceType) => [deviceType.id, deviceType]),
  )
  ...
  const linkedModels = models
    .map((model) => {
      const sourceValues = [
        ...new Set(
          (refsByTarget.get(model.id) ?? []).map(
            (reference) => reference.sourceValue,
          ),
        ),
      ]
      const existingSuggestion = model.familyId
        ? null
        : suggestImportModelFamily(
      ...
      const proposedNewFamilyName =
        model.familyId || existingSuggestion
          ? null
          : suggestNewImportModelFamilyName(
              `${sourceValues[0] ?? ''} ${model.model}`,
              model.vendor.name,
              model.vendor.code,
            )
      return {
    ...
    .sort(
      (left, right) =>
        left.vendor.name.localeCompare(right.vendor.name) ||
        left.model.localeCompare(right.model),
    )

  const classificationByReference = new Map(
    references.map((reference) => [
      reference.id,
      classifyImportedDeviceModel(reference.sourceValue, normalizationRules),
    ]),
  )
  const predictionByReference = new Map(
    references.map((reference) => {
    ...
    return [
      {
        ...
        proposedModel: applyDeviceImportModelTransforms(
        ...
        preferredSoftwarePlatform:
          applied.prediction.preferredSoftwarePlatform ?? null,
      },
    ]
  })
  const readyToCreate = references
    .filter((reference) => {
      ...
      const classifiedType = classification
        ? deviceTypes.find(
            (item) =>
              normalizeImportText(item.name) ===
              normalizeImportText(classification.deviceTypeName),
          )
        : null
      const vendorId = prediction?.vendorTargetId ?? meta.vendorTargetId
      const deviceTypeId =
        prediction?.deviceTypeTargetId ?? meta.deviceTypeTargetId
      return Boolean(
        vendorId &&
        vendorById.get(vendorId)?.isActive &&
        (typeById.get(deviceTypeId ?? '')?.isActive ||
          classifiedType?.isActive),
      )
    })
    .map((reference) => {
      const meta = metadata(reference.metadata)
      ...
      const vendor = vendorById.get(
        (prediction?.vendorTargetId ?? meta.vendorTargetId)!,
      )!
      const classifiedType = classification
        ? deviceTypes.find(
            (item) =>
        ...
        : null
      const deviceType =
        typeById.get(prediction?.deviceTypeTargetId ?? '') ??
        classifiedType ??
        typeById.get(meta.deviceTypeTargetId!)!
      const classifiedFamily = classification
        ? families.find(
            (item) =>
              item.vendorId === vendor.id &&
              normalizeImportText(item.name) ===
                normalizeImportText(classification.productFamilyName),
          )
        : null
      const predictedFamily = prediction?.productFamilyId
        ? families.find(
            (item) =>
              item.id === prediction.productFamilyId &&
              item.vendorId === vendor.id,
          )
        : null
      const existingSuggestion =
        predictedFamily ??
        classifiedFamily ??
        suggestImportModelFamily(reference.sourceValue, vendor.id, families)
      const inferredPlatforms = cleanPlatforms(
        [
          ...(meta.platforms ?? []),
          ...(prediction?.softwarePlatforms ?? []),
          ...(classification?.softwarePlatforms.map(
            softwarePlatformLegacyValue,
          ) ?? []),
        ],
        prediction?.preferredSoftwarePlatform ?? meta.platform ?? null,
      )
      const preferredPlatform = prediction?.preferredSoftwarePlatform
        ...
        : classification?.preferredSoftwarePlatformCode
          ? classification.softwarePlatforms.find(
              (item) =>
                item.code === classification.preferredSoftwarePlatformCode,
            )
          : null
      const proposedModel = applyDeviceImportModelTransforms(
        classification?.model ?? reference.sourceValue,
        prediction?.modelTransforms,
      )
      return {
        id: reference.id,
        ...
        proposedModel,
        proposedPlatform:
          typeof preferredPlatform === 'string'
            ? preferredPlatform
            : preferredPlatform
              ? softwarePlatformLegacyValue(preferredPlatform)
              : inferredPlatforms.length === 1
                ? inferredPlatforms[0]
                : '',
        proposedPlatforms: inferredPlatforms,
        suggestedFamilyId: existingSuggestion?.id ?? null,
        suggestedFamilyName: existingSuggestion?.name ?? null,
        proposedNewFamilyName: existingSuggestion
          ? null
          : (classification?.productFamilyName ??
            suggestNewImportModelFamilyName(
              reference.sourceValue,
              vendor.name,
              vendor.code,
  ...
  const groupedNewFamilies = new Map<
    string,
    {
      key: string
      vendorId: string
  ...
  for (const model of linkedModels) {
    if (
      model.familyId ||
      model.suggestedFamilyId ||
      !model.proposedNewFamilyName
    )
      continue
  ...
  return {
    ...
    linkedModels,
    families,
    newFamilyProposals: [...groupedNewFamilies.values()],
  }
}

function parseModelCreateItems(
    ...
    return rawItems.map((rawItem) => {
      ...
      const model =
        typeof item.model === 'string'
          ? item.model.normalize('NFKC').trim().replace(/\s+/g, ' ')
          : ''
      ...
      const familyId =
        typeof item.familyId === 'string' && item.familyId.trim()
          ? item.familyId.trim()
          : null
      if (!referenceId || !model)
        throw new DeviceImportStagingError(
          'Every prepared Model needs a staged reference and concrete Model name.',
        )
      if (model.length > 160)
        throw new DeviceImportStagingError(
          `Model “${model}” is longer than 160 characters.`,
        )
      if (platform && platform.length > 160)
        throw new DeviceImportStagingError(
          `Platform “${platform}” is longer than 160 characters.`,
        )
      if (platforms.some((value) => value.length > 160))
        throw new DeviceImportStagingError(
          'Supported platform names must be 160 characters or fewer.',
        )
      return { referenceId, model, platform, platforms, familyId }
    })
  }
  const referenceIds = Array.isArray(rawInput.referenceIds)
    ? rawInput.referenceIds
        .filter(
  ...
  return referenceIds.map((referenceId) => {
    ...
    const platforms = cleanPlatforms(
      meta.platforms ?? [],
      meta.platform ?? null,
    )
    return {
...
export async function bulkCreateDeviceImportModels(rawInput: unknown) {
  ...
  if (!items.length)
    throw new DeviceImportStagingError(
      'Choose at least one prepared Model to create.',
    )
  if (items.length > MAX_BULK_MODELS)
    throw new DeviceImportStagingError(
      `Create at most ${MAX_BULK_MODELS} Models in one bulk action.`,
    )
  if (new Set(items.map((item) => item.referenceId)).size !== items.length)
    throw new DeviceImportStagingError(
      'A staged Model can only appear once in a bulk create action.',
    )

  const referenceById = new Map(
    allUnresolved.map((reference) => [reference.id, reference]),
  )
  if (items.some((item) => !referenceById.has(item.referenceId)))
    throw new DeviceImportStagingError(
      'One or more selected Models are no longer unresolved or ready to create.',
    )
  ...
  if (
    ...
    throw new DeviceImportStagingError(
      'Resolve Vendor and Device Type before bulk-creating Models.',
    )

  const vendorIds = [
    ...new Set(
      references.map(
        (reference) => metadata(reference.metadata).vendorTargetId!,
      ),
    ),
  ]
  const typeIds = [
    ...new Set(
      references.map(
        (reference) => metadata(reference.metadata).deviceTypeTargetId!,
      ),
    ),
  ]
  const familyIds = [
  ...
  if (vendors.length !== vendorIds.length)
    throw new DeviceImportStagingError(
      'One or more resolved Vendors no longer exist or are archived.',
    )
  if (types.length !== typeIds.length)
    throw new DeviceImportStagingError(
      'One or more resolved Device Types no longer exist or are archived.',
    )
  ...
  for (const item of items) {
    ...
    if (item.familyId) {
      ...
      if (!family || family.vendorId !== vendorId)
        throw new DeviceImportStagingError(
          `The selected family for “${item.model}” does not belong to the resolved Vendor.`,
        )
    }
    ...
    const exact = existingModels.find(
      (model) =>
        ...
        normalizeImportText(model.model) === normalized,
    )
    if (exact) {
      if (exact.deviceTypeId !== deviceTypeId)
        throw new DeviceImportStagingError(
          `Model “${item.model}” already exists for this Vendor under another Device Type.`,
        )
      links.push({
    ...
    if (current) {
      if (current.deviceTypeId !== deviceTypeId)
        throw new DeviceImportStagingError(
          `Model “${item.model}” resolves to conflicting Device Types in this import.`,
        )
      if (current.familyId !== item.familyId)
        throw new DeviceImportStagingError(
          `Model “${item.model}” has conflicting Family proposals. Make the prepared rows consistent.`,
        )
      if (
        current.platform &&
        item.platform &&
        normalizeImportText(current.platform) !==
          normalizeImportText(item.platform)
      ) {
        throw new DeviceImportStagingError(
          `Model “${item.model}” has conflicting preferred-platform proposals. Leave preferred platform blank for a dual-platform model.`,
        )
      }
      if (!current.platform && item.platform) current.platform = item.platform
      for (const platform of item.platforms)
        current.platforms.set(normalizeImportText(platform), platform)
  ...
  const platformCreates = [
    ...pending.flatMap((model) =>
      [...model.platforms.values()].map((platform) => ({
  ...
  const operations = [
    ...(pending.length
      ? [
          prisma.deviceModel.createMany({
            data: pending.map((model) => ({
              ...
              platform:
                model.platform ??
                (model.platforms.size === 1
                  ? [...model.platforms.values()][0]
                  : null),
      ...
      : []),
    ...(platformCreates.length
      ? [
          prisma.deviceModelPlatform.createMany({
            data: platformCreates,
            skipDuplicates: true,
          }),
        ]
      : []),
    ...links
      .filter((link) => link.preferred)
      .map((link) =>
        prisma.deviceModel.updateMany({
          where: { id: link.targetId, platform: null },
          data: { platform: link.preferred },
        }),
      ),
    ...pending.flatMap((model) =>
      model.referenceIds.map((referenceId) =>
    ...
    ...links.map((link) =>
      prisma.deviceImportStagedReference.update({
        ...
        data: {
          ...
          suggestedTargetId: null,
          suggestionScore: null,
          resolutionSource: link.source,
        },
      }),
    ),
  ]
  await prisma.$transaction(operations)
  await synchronizeModelSoftwarePlatforms([
    ...pending.map((model) => model.id),
    ...links.map((link) => link.targetId),
  ])
  const workspace = deferRefresh
    ? null
...
export async function bulkAssignDeviceImportModelFamilies(rawInput: unknown) {
  ...
  if (!rawItems.length)
    throw new DeviceImportStagingError(
      'Choose at least one Model family assignment.',
    )
  if (rawItems.length > MAX_FAMILY_ASSIGNMENTS)
    throw new DeviceImportStagingError(
      `Update at most ${MAX_FAMILY_ASSIGNMENTS} Model families in one action.`,
    )
  await assertMutableBatch(batchId)
  ...
  const items = rawItems.map((rawItem) => {
    ...
    if (!modelId || !familyId)
      throw new DeviceImportStagingError(
        'Every Model family assignment needs a Model and Family.',
      )
    return { modelId, familyId }
  })
  if (new Set(items.map((item) => item.modelId)).size !== items.length)
    throw new DeviceImportStagingError(
      'A Model can only appear once in a family update.',
    )
  ...
  const allowedTargets = await prisma.deviceImportStagedReference.findMany({
    where: {
      ...
      kind: 'DEVICE_MODEL',
      status: 'LINKED',
      targetId: { in: items.map((item) => item.modelId) },
    },
  ...
  if (items.some((item) => !allowedIds.has(item.modelId)))
    throw new DeviceImportStagingError(
      'Only Models linked in this staged import can be bulk-updated here.',
    )

  const [models, families] = await Promise.all([
    prisma.deviceModel.findMany({
      where: { id: { in: items.map((item) => item.modelId) } },
      select: { id: true, vendorId: true, familyId: true },
    }),
    prisma.deviceModelFamily.findMany({
      where: { id: { in: items.map((item) => item.familyId) }, isActive: true },
      select: { id: true, vendorId: true },
    }),
  ])
  ...
  for (const item of items) {
    ...
    if (!model || !family)
      throw new DeviceImportStagingError(
        'One or more Models or Families no longer exist or are archived.',
      )
    if (model.vendorId !== family.vendorId)
      throw new DeviceImportStagingError(
        'A Model family must belong to the same Vendor as the Model.',
      )
  }

  const changed = items.filter(
    (item) => modelsById.get(item.modelId)?.familyId !== item.familyId,
  )
  await prisma.$transaction(
    changed.map((item) =>
      prisma.deviceModel.update({
...
export async function bulkCreateAndAssignDeviceImportModelFamilies(
  rawInput: unknown,
) {
  const input =
    typeof rawInput === 'object' && rawInput !== null
  ...
  if (!rawItems.length)
    throw new DeviceImportStagingError(
      'Choose at least one proposed Model family.',
    )
  if (rawItems.length > MAX_FAMILY_ASSIGNMENTS)
    throw new DeviceImportStagingError(
      `Create at most ${MAX_FAMILY_ASSIGNMENTS} Model families in one action.`,
    )
  await assertMutableBatch(batchId)
  ...
  const items = rawItems.map((rawItem) => {
    const item =
      typeof rawItem === 'object' && rawItem !== null
        ...
        : {}
    const vendorId =
      typeof item.vendorId === 'string' ? item.vendorId.trim() : ''
    const name =
      typeof item.name === 'string'
        ? item.name.normalize('NFKC').trim().replace(/\s+/g, ' ')
        : ''
    const modelIds = Array.isArray(item.modelIds)
      ? item.modelIds
          ...
          .map((value) => value.trim())
      : []
    if (!vendorId || !name || !modelIds.length)
      throw new DeviceImportStagingError(
        'Every proposed Family needs a Vendor, Family name, and at least one Model.',
      )
    return { vendorId, name, modelIds }
  })
  const familyKeys = items.map(
    (item) => `${item.vendorId}|${normalizeImportText(item.name)}`,
  )
  if (new Set(familyKeys).size !== familyKeys.length)
    throw new DeviceImportStagingError(
      'Two proposed Families resolve to the same name for the same Vendor. Merge or rename them first.',
    )
  const allModelIds = items.flatMap((item) => item.modelIds)
  if (new Set(allModelIds).size !== allModelIds.length)
    throw new DeviceImportStagingError(
      'A Model can only be assigned by one proposed Family in this action.',
    )
  ...
  const [linkedReferences, models, existingFamilies] = await Promise.all([
    ...
    prisma.deviceModelFamily.findMany({
      where: {
        vendorId: { in: [...new Set(items.map((item) => item.vendorId))] },
      },
      select: { id: true, vendorId: true, name: true, isActive: true },
    }),
  ])
  ...
  if (allModelIds.some((id) => !linkedIds.has(id) || !modelById.has(id)))
    throw new DeviceImportStagingError(
      'Only Models linked in this staged import can receive proposed Families.',
    )
  ...
  for (const item of items) {
    for (const modelId of item.modelIds) {
      ...
      if (model.vendorId !== item.vendorId)
        throw new DeviceImportStagingError(
          `Proposed Family “${item.name}” contains a Model from another Vendor.`,
        )
      if (model.familyId)
        throw new DeviceImportStagingError(
          'A proposed new Family cannot overwrite an existing Model family assignment.',
        )
    }
    const existing = existingFamilies.find(
      (family) =>
        family.vendorId === item.vendorId &&
        normalizeImportText(family.name) === normalizeImportText(item.name),
    )
    if (existing) {
      if (!existing.isActive)
        throw new DeviceImportStagingError(
          `Family “${item.name}” already exists but is archived. Reactivate or choose another Family.`,
        )
  ...
  const operations = [
    ...(createdFamilies.length
      ? [
          prisma.deviceModelFamily.createMany({
            data: createdFamilies.map((family) => ({
              id: family.id,
              vendorId: family.vendorId,
              name: family.name,
              isActive: true,
              notes: 'Created from staged XLSX inventory import suggestion.',
            })),
          }),
        ]
      : []),
    ...familyAssignments.flatMap((assignment) =>
      assignment.modelIds.map((modelId) =>
        prisma.deviceModel.update({
          where: { id: modelId },
