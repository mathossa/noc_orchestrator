...
export async function getDeviceImportModelAssist(batchId: string) {
  ...
  const rulePredictions = references.flatMap((reference) => {
    ...
    return [{
      ...
      proposedModel: applyDeviceImportModelTransforms(reference.sourceValue, applied.prediction.modelTransforms),
  ...
  const readyToCreate = references.filter((reference) => {
    ...
    return {
      ...
      proposedModel: applyDeviceImportModelTransforms(classification?.model ?? reference.sourceValue, prediction?.modelTransforms),
